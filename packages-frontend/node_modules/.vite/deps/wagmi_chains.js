import { $ as Field, $n as sha256, $o as parseAccount, Aa as toRlp, An as concat, At as AccountNotFoundError, B as toRpcList$1, Bt as readContract, Cn as size, Cs as keccak256$1, Ct as formatTransactionReceipt$1, Dc as getAction, Dn as toString, Dt as sendTransaction, En as toNumber, Et as writeContract, Fn as BaseError$1, Fs as hexToBigInt, G as fromRpc$5, Gs as pad, H as extract$1, Hn as isAddressEqual, I as recoverAddress, J as toRpc$5, Jn as getTransactionType, K as fromTuple$3, L as recoverPublicKey, Ls as hexToNumber$1, Mn as fromString$1, Nn as isEqual, Ns as toHex, On as trimLeft, Os as toBytes$1, Pn as stringify, Q as createCurve, R as verify$4, Sc as BaseError, Sn as random, St as defineTransactionReceipt, Tn as toBigInt, Ts as hexToBytes, U as from$7, V as assert$2, Vn as formatLog, W as fromHex$1, Wt as getCallError, X as yParityToV, Y as toTuple$3, Zo as encodeFunctionData, Zt as InvalidChainIdError$1, _n as from$6, _t as validate$1, bi as maxUint256, bn as fromString, br as assertRequest, ct as checksum, d as defineChain, da as formatTransactionRequest$1, dr as defineBlock, dt as isEqual$1, er as sha256$1, et as fromHex$2, f as extendSchema, ft as validate, ga as FeeCapTooHighError$1, gn as concat$1, gt as sha256$2, hn as assert$3, hs as isAddress, ht as keccak256, j as verifyHash$1, jn as fromHex, js as numberToHex$1, kn as validate$2, lt as from$5, mr as formatTransaction$1, ms as concatHex, mt as toHex$1, n as toYParitySignatureArray, nr as sha512, oi as maxUint16, ot as getSelector, pa as extract, pr as defineTransaction, pt as fromHex$3, q as toHex$3, r as serializeAccessList, st as assert$4, t as serializeTransaction$3, tr as sha384, tt as toHex$2, ua as defineTransactionRequest, ut as fromPublicKey, vn as fromBytes, wa as TipAboveFeeCapError$1, wn as slice, xn as padLeft, xt as getCode, yn as fromNumber, ys as InvalidAddressError, z as fromRpcList$1, zn as parseEventLogs, zs as trim } from "./serializeTransaction-C-XFMBWp.js";
...
//#endregion
//#region node_modules/viem/_esm/chains/definitions/citrate.js
var citrate = /*#__PURE__*/ defineChain({
	id: 40204,
	name: "Citrate",
	nativeCurrency: {
		name: "SALT",
...
//#endregion
//#region node_modules/viem/_esm/chains/definitions/grav.js
var grav = /*#__PURE__*/ defineChain({
	id: 127001,
	name: "Gravity",
	nativeCurrency: {
		name: "Gravity",
...
//#endregion
//#region node_modules/viem/_esm/chains/definitions/ladyChain.js
var ladyChain = /*#__PURE__*/ defineChain({
	id: 589,
	name: "LadyChain",
	nativeCurrency: {
		name: "Lady",
	...
	rpcUrls: { default: { http: ["https://ladyrpc.us/rpc"] } },
	blockExplorers: { default: {
		name: "LadyScan",
...
*/
function assert$1(config) {
	const { salt, threshold, owners } = config;
	if (typeof salt !== "undefined" && size(salt) !== 32) throw new InvalidConfigError({ reason: "salt must be 32 bytes" });
	if (owners.length === 0) throw new InvalidConfigError({ reason: "owners cannot be empty" });
	if (owners.length > 10) throw new InvalidConfigError({ reason: "too many owners" });
	if (Number(threshold) < 1) throw new InvalidConfigError({ reason: "threshold cannot be zero" });
	let totalWeight = 0;
	let previous;
	for (const owner of owners) {
		if (!validate(owner.owner) || toBigInt(owner.owner) === 0n) throw new InvalidConfigError({ reason: "owner cannot be zero" });
		if (Number(owner.weight) < 1) throw new InvalidConfigError({ reason: "owner weight cannot be zero" });
		const current = toBigInt(owner.owner);
		if (typeof previous !== "undefined" && previous >= current) throw new InvalidConfigError({ reason: "owners must be strictly ascending" });
		previous = current;
		totalWeight += Number(owner.weight);
	}
	if (totalWeight > 4294967295) throw new InvalidConfigError({ reason: "total owner weight exceeds u32 max" });
	if (Number(threshold) > totalWeight) throw new InvalidConfigError({ reason: "threshold exceeds total owner weight" });
}
/**
...
*/
function from$4(config) {
	const owners = [...config.owners].sort((a, b) => toBigInt(a.owner) < toBigInt(b.owner) ? -1 : 1);
	const normalized = {
		salt: config.salt ? padLeft(config.salt, 32) : zeroSalt,
...
function fromTuple$2(tuple) {
	...
	return {
		salt: salt && salt !== "0x" ? padLeft(salt, 32) : zeroSalt,
...
function getAddress(value) {
	...
	return from$5(slice(keccak256(concat$1(fromString(accountDomain), id)), 12, 32));
}
/**
...
*/
function getSignPayload$2(value) {
	const { payload } = value;
	...
	return keccak256(concat$1(fromString(signatureDomain), from$6(payload), account, genesisConfigId));
}
/**
...
function toId(config) {
	assert$1(config);
	const id = keccak256(concat$1(fromString(configDomain), padLeft(config.salt ?? zeroSalt, 32), fromNumber(config.threshold, { size: 4 }), fromNumber(config.owners.length, { size: 4 }), ...config.owners.flatMap((owner) => [owner.owner, fromNumber(owner.weight, { size: 4 })])));
	if (toBigInt(id) === 0n) throw new InvalidConfigError({ reason: "config ID cannot be zero" });
	return id;
}
/**
...
function toTuple$2(config) {
	assert$1(config);
	const owners = config.owners.map((owner) => [owner.owner, fromNumber(owner.weight)]);
	return [
		config.salt ? padLeft(config.salt, 32) : zeroSalt,
		fromNumber(config.threshold),
		owners
	];
}
/** Thrown when a native multisig config is invalid. */
var InvalidConfigError = class extends BaseError$1 {
	constructor({ reason }) {
...
function extractPublicKey(options) {
	...
	switch (signature.type) {
		...
		case "multisig": throw new CoercionError({ envelope: signature });
	}
}
/**
...
function deserialize$1(value) {
	...
	if (typeId === serializedMultisigType) {
		const [account, genesisConfigId, signatures, init] = toHex$2(data);
		return {
			type: "multisig",
			account,
			genesisConfigId,
			signatures: signatures.map((signature) => deserialize$1(signature)),
			...init && init !== "0x" ? { init: fromTuple$2(init) } : {}
		};
	}
...
function from$3(value, options) {
	...
	if (type === "multisig") {
		const { genesisConfig, init, ...rest } = value;
		const account = (() => {
			if (rest.account) return rest.account;
			if (genesisConfig) return getAddress(genesisConfig);
			return rest.account;
		})();
		const genesisConfigId = (() => {
			if (rest.genesisConfigId) return rest.genesisConfigId;
			if (genesisConfig) return toId(genesisConfig);
		...
		return {
			...
			account,
			genesisConfigId,
			signatures: rest.signatures.map((signature) => from$3(signature)),
			...initSource ? { init: from$4(initSource) } : {},
			type
		};
	}
	return {
...
function fromRpc$4(envelope) {
	...
	if (envelope.type === "multisig" || "account" in envelope && "configId" in envelope && "signatures" in envelope) {
		...
		return {
			...
			...multisig.init ? { init: from$4(multisig.init) } : {}
		};
	}
...
function getType$1(envelope) {
	...
	if (("account" in envelope && "genesisConfigId" in envelope || "genesisConfig" in envelope) && "signatures" in envelope) return "multisig";
	throw new CoercionError({ envelope });
}
/**
...
function serialize$2(envelope, options = {}) {
	...
	if (type === "secp256k1") return concat$1(toHex$3(envelope.signature), options.magic ? magicBytes : "0x");
	if (type === "p256") {
		const p256 = envelope;
	...
	if (type === "multisig") {
		const multisig = envelope;
		return concat$1(serializedMultisigType, fromHex$2([
			multisig.account,
			multisig.genesisConfigId,
			multisig.signatures.map((signature) => serialize$2(signature)),
...
function sortMultisigApprovals(value) {
	...
	const digest = getSignPayload$2("genesisConfig" in value && value.genesisConfig ? {
		payload,
		genesisConfig: value.genesisConfig
	} : {
		payload,
...
function toRpc$4(envelope) {
	...
	if (type === "multisig") {
		...
		return {
			...
			signatures: multisig.signatures.map((signature) => serialize$2(signature)),
...
function fromTupleList(accessList) {
	...
	for (let i = 0; i < accessList.length; i++) {
		...
		if (address) assert$4(address, { strict: false });
		list.push({
			address,
			storageKeys: storageKeys.map((key) => validate$1(key) ? key : trimLeft(key))
		});
	}
	return list;
}
...
function assert(envelope) {
	...
	if (calls) {
		for (const call of calls) if (call.to) assert$4(call.to, { strict: false });
	}
...
function deserialize(serialized) {
	const transactionArray = toHex$2(slice(serialized, 1));
	...
	if (validate$2(gas) && gas !== "0x") transaction.gas = BigInt(gas);
	if (validate$2(nonce)) transaction.nonce = nonce === "0x" ? 0n : BigInt(nonce);
	if (validate$2(maxFeePerGas) && maxFeePerGas !== "0x") transaction.maxFeePerGas = BigInt(maxFeePerGas);
	if (validate$2(maxPriorityFeePerGas) && maxPriorityFeePerGas !== "0x") transaction.maxPriorityFeePerGas = BigInt(maxPriorityFeePerGas);
	if (validate$2(nonceKey)) transaction.nonceKey = nonceKey === "0x" ? 0n : BigInt(nonceKey);
	if (validate$2(validBefore) && validBefore !== "0x") transaction.validBefore = Number(validBefore);
	if (validate$2(validAfter) && validAfter !== "0x") transaction.validAfter = Number(validAfter);
	if (validate$2(feeToken) && feeToken !== "0x") transaction.feeToken = feeToken;
	...
	if (feePayerSignatureOrSender !== "0x" && feePayerSignatureOrSender !== void 0) if (feePayerSignatureOrSender === "0x00" || validate(feePayerSignatureOrSender)) {
		transaction.feePayerSignature = null;
		if (validate(feePayerSignatureOrSender)) transaction.from = feePayerSignatureOrSender;
	} else transaction.feePayerSignature = fromTuple$3(feePayerSignatureOrSender);
...
function from(envelope, options = {}) {
	...
	return {
		...
		...feePayerSignature ? { feePayerSignature: from$7(feePayerSignature) } : {},
		type: "tempo"
	};
}
/**
...
async function serializeTempo(transaction, sig) {
	...
	if (transaction.multisig && transaction.signatures && !feePayer) {
		const sorted = sortMultisigApprovals({
			payload: getSignPayload(from(transaction_ox)),
			signatures: transaction.signatures.map((approval) => from$3(approval)),
			genesisConfig: transaction.multisig
		});
		return serialize$1(transaction_ox, {
	...
	if (signature && typeof transaction.feePayer === "object") {
		...
		return serialize$1(tx, { feePayerSignature: from$7(await transaction.feePayer.sign({ hash })) });
	}
...
var chainConfig = {
	...
	prepareTransactionRequest: [async (r, { client, phase }) => {
		...
		const multisig = request.multisig ?? (request.account?.source === "multisig" ? request.account.config : void 0);
		if (multisig) {
			request.multisig = multisig;
			request.from = getAddress(multisig);
...
//#endregion
//#region node_modules/viem/_esm/chains/definitions/valygoNft.js
var valygoNft = defineChain({
	...
	rpcUrls: { default: {
		http: ["https://rpc-gw-1.vyoscan.com/ext/bc/2RyzsmGypNQZPby1miwMMV8spTvhgd9qd2peNRzU1mErUQqSSw/rpc", "https://rpc-gw-2.vyoscan.com/ext/bc/2RyzsmGypNQZPby1miwMMV8spTvhgd9qd2peNRzU1mErUQqSSw/rpc"],
		webSocket: ["wss://ws.vyoscan.com/ext/bc/2RyzsmGypNQZPby1miwMMV8spTvhgd9qd2peNRzU1mErUQqSSw/ws"]
	} },
	blockExplorers: { default: {
		name: "VYOScan NFT",
	...
	contracts: { multicall3: { address: "0x4469693a04BAE365795329a89b24F4Eb1303Ea08" } }
});
//#endregion
//#region node_modules/viem/_esm/chains/definitions/valygoSmartchain.js
var valygoSmartchain = defineChain({
	...
	rpcUrls: { default: {
		http: ["https://rpc-gw-1.vyoscan.com/ext/bc/2t51dXsuxUvd9teY9TKEJmgxmxMk3CRF88UYTA4HQgjeYZqzSX/rpc", "https://rpc-gw-2.vyoscan.com/ext/bc/2t51dXsuxUvd9teY9TKEJmgxmxMk3CRF88UYTA4HQgjeYZqzSX/rpc"],
		webSocket: ["wss://ws.vyoscan.com/ext/bc/2t51dXsuxUvd9teY9TKEJmgxmxMk3CRF88UYTA4HQgjeYZqzSX/ws"]
	} },
	blockExplorers: { default: {
		name: "VYOScan",
	...
	contracts: { multicall3: { address: "0xeFa3c632BD275750597cE9ca2346A5becAA0F344" } }
});
//#endregion
...
//#endregion
//#region node_modules/viem/_esm/chains/definitions/whitechainSepolia.js
var whitechainSepolia = /*#__PURE__*/ defineChain({
	testnet: true,
	id: 1874,
